...
export default {
  ...
  methods: {
    ...
    elementDown(e) {
      ...
      if (this.$el.contains(target)) {
        ...
        if (!this.enabled) {
          ...
          //按下shift时 e.shiftKey为true，否则为false
          //当按下shift时，点击piece可以切换选中状态
          this.enabled = false;
    ...
    deselect(e) {
      ...
      //scissor为切割按钮的className del为删除按钮的className
      if (
        !this.$el.contains(target) &&
        !regex.test(target.className) &&
        target.className != "scissor" &&
    ...
    handleDown(handle, e) {
      if (this.onResizeStart) {
        let maxValues = this.onResizeStart(handle, e);
        ...
        if (
          maxValues != null &&
          typeof maxValues === "object" &&
          Array.isArray(maxValues) === false
        ) {
          if (maxValues.maxWidth) {
            this.maxW = maxValues.maxWidth;
          }

          /* not used
          if (maxValues.maxHeight) {
            this.maxH = maxValues.maxHeight;
          }
          if (maxValues.minWidth) {
            this.minW = maxValues.minWidth;
          }
          if (maxValues.minHeight) {
            this.minH = maxValues.minHeight;
          }//*/
        }
      }
  ...
  computed: {
    style() {
      return {
        ...
        border: this.enabled ? "3px solid #F9B703" : "", //边框选中样式
        zIndex: this.zIndex,
